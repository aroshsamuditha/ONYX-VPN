...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY') or 'dev-only-change-with-SECRET_KEY-in-production'
...
def subscribe(plan_id):
    ...
    plan_row = db.execute_one('SELECT * FROM plans WHERE id = ?', (plan_id,))
    if not plan_row:
...
def cron_expire_subscriptions():
    ...
    Set CRON_SECRET in the environment and pass ?token=... or X-Cron-Secret header."""
...
# Run expiry on import so gunicorn/docker workers refresh state at startup
...
if __name__ == '__main__':
    ...
    app.run(debug=debug, host='0.0.0.0', port=_parse_port())
